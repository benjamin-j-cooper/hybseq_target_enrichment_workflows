...
helptext= '''
####Note from Diego F. Morales-Briones####
...
def get_concord_and_conflict(phyparts_root,subtrees_dict,subtrees_topids):
	...
	with open(phyparts_root + ".concon.tre") as phyparts_trees:
		...
		total_node = Tree(phyparts_trees.readline())
	...
	for node in total_node.traverse():
		node_topid = node.get_topology_id()
		for subtree in subtrees_dict:
			if node_topid == subtrees_topids[subtree]:
				total_node_dict[subtree] = node.support
				
				
	return concord_dict, conflict_dict, total_node_dict    
...
def get_pie_chart_data(phyparts_root,total_genes,concord_dict,conflict_dict):
	...
	for n in phyparts_hist:
		...
		total_node = total_node_dict[node_name]
		...
		unsuported = ((total_genes - concord - all_conflict) - ((total_genes - concord - all_conflict) - (total_node - concord - all_conflict)))/ total_genes * 100
		missing = ((total_genes - concord - all_conflict) - (total_node - concord - all_conflict)) / total_genes * 100
	
		pie_list = [adj_concord,adj_most_conflict,other_conflict,unsuported,missing]
...
parser.add_argument("--colors",help="Four colors of the pie chart: concordance (blue) top conflict (green), other conflict (red), no signal (gray), missing (silver	)",nargs="+",default=["blue","green","red","gray","silver"])	
args = parser.parse_args()
plot_tree,subtrees_dict,subtrees_topids = get_phyparts_nodes(args.species_tree, args.phyparts_root)
#print(subtrees_dict)
concord_dict, conflict_dict, total_node_dict = get_concord_and_conflict(args.phyparts_root,subtrees_dict,subtrees_topids)
