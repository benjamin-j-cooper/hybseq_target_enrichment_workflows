...
This script plots the piecharts proportional to the total number of informative trees (i.e. node present in gene tree) for each node.
The original script ask for fix number of total genes.
...
def get_concord_and_conflict(phyparts_root,subtrees_dict,subtrees_topids):
	...
	with open(phyparts_root + ".concon.tre") as phyparts_trees:
		...
		total_tree = Tree(phyparts_trees.readline())
	...
	for node in total_tree.traverse():
		node_topid = node.get_topology_id()
		for subtree in subtrees_dict:
			if node_topid == subtrees_topids[subtree]:
				total_dict[subtree] = node.support
				
				
	return concord_dict, conflict_dict, total_dict    
    
#Generate Pie Chart data
#def get_pie_chart_data(phyparts_root,total_genes,concord_dict,conflict_dict):
def get_pie_chart_data(phyparts_root,concord_dict,conflict_dict):
	...
	for n in phyparts_hist:
		...
		total_genes = total_dict[node_name]
...
#print(subtrees_dict)
concord_dict, conflict_dict, total_dict = get_concord_and_conflict(args.phyparts_root,subtrees_dict,subtrees_topids)
#phyparts_dist, phyparts_pies = get_pie_chart_data(args.phyparts_root,args.num_genes,concord_dict,conflict_dict)
phyparts_dist, phyparts_pies = get_pie_chart_data(args.phyparts_root,concord_dict,conflict_dict)
