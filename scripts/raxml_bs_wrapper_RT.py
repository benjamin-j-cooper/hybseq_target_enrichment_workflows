...
def raxml_bs(DIR,cleaned,num_cores,seqtype,replicates=100):
	...
	tree = DIR+clusterID+"."+(cleaned.split(".")[1])+"."+(cleaned.split(".")[2])+".raxml_bs.tre"
	...
	try:
		...
		os.rename("RAxML_bootstrap."+cleaned, DIR+clusterID+"."+(cleaned.split(".")[1])+"."+(cleaned.split(".")[2])+".raxml_bs.trees")
