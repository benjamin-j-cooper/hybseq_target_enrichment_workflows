	...
	for x in os.listdir(tree_DIR):
		if x.endswith(tree_file_ending):
			...
			if len(tips) != len(set(tips)):
				...
				clade_list=list(dict.fromkeys(df['Clade'].values.tolist()))		
				
				check =  all(item in tips for item in clade_list)

				if check is True:
					
					with open(outDIR+base_name_tree[0]+".all.mono","w") as outfile:
						outfile.write(str(parsed)+";\n")			
				else:
					with open(outDIR+base_name_tree[0]+".mis.mono","w") as outfile:
						outfile.write(str(parsed)+";\n")
