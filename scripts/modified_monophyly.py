...
def rename_tips(tree_DIR, tree_file_ending, clade_assignments, outDIR):
	...
	for x in os.listdir(tree_DIR):
		if x.endswith(tree_file_ending):
			...
			tips2 = []
			for y in tips: tips2.append(y.split("@")[0])
		
			if len(tips2) != len(set(tips2)): print "Clades not monophyletic"
			
			else:
				
				for leaf in t.get_terminals(): leaf.name = leaf.name.split("@")[0]
				Phylo.write(t, outDIR+base_name_tree[0]+".mt", 'newick')
